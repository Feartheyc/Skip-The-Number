...
const Game1 = {
  ...
  ============================================================ */
  _buildTorusCache(Ro, Ri) {
    const diam = Math.ceil((Ro + 20) * 2);
    // Use OffscreenCanvas if available, otherwise a regular canvas
    let oc;
    if (typeof OffscreenCanvas !== "undefined") {
      oc = new OffscreenCanvas(diam, diam);
    } else {
      oc = document.createElement("canvas");
      oc.width = oc.height = diam;
    }
    const octx  = oc.getContext("2d");
    const hw    = diam / 2;   // half-width = centre of offscreen canvas
    const Rm    = (Ro + Ri) / 2;
    const r     = (Ro - Ri) / 2;

    // Pick a fixed "neutral" light direction for the static cache
    const lx = Math.cos(0), ly = Math.sin(0);

    octx.translate(hw, hw);

    // 1. Ambient nebula bloom
    const bloom = octx.createRadialGradient(0, 0, Ri - r * 2.5, 0, 0, Ro + r * 3.5);
    ...
    octx.beginPath();
    octx.arc(0, 0, Ro + r * 3.5, 0, Math.PI * 2);
    octx.fill();

    // 2. Tube body fill
    const gx0 = -lx * (Ro + r), gy0 = -ly * (Ro + r);
    const gx1 =  lx * (Ro + r), gy1 =  ly * (Ro + r);
    const tubeGrad = octx.createLinearGradient(gx0, gy0, gx1, gy1);
    ...
    octx.beginPath();
    octx.arc(0, 0, Ro, 0, Math.PI * 2);
    octx.arc(0, 0, Ri, 0, Math.PI * 2, true);
    octx.fillStyle  = tubeGrad;
    octx.shadowColor = "rgba(201,147,58,0.4)";
    octx.shadowBlur  = 36;
    octx.fill("evenodd");
    octx.shadowBlur  = 0;

    // 3. Inner hole darkness
    const holeDark = octx.createRadialGradient(0, 0, Ri * 0.65, 0, 0, Ri);
    ...
    octx.beginPath();
    octx.arc(0, 0, Ri, 0, Math.PI * 2);
    octx.fillStyle = holeDark;
    octx.fill();

    // 4. Outer edge stroke
    octx.beginPath();
    octx.arc(0, 0, Ro, 0, Math.PI * 2);
    octx.strokeStyle = "#d4a44a"; octx.lineWidth = 2.8;
    octx.shadowColor = "rgba(212,164,74,0.6)"; octx.shadowBlur = 18;
    octx.stroke(); octx.shadowBlur = 0;

    // 5. Inner edge stroke
    octx.beginPath();
    octx.arc(0, 0, Ri, 0, Math.PI * 2);
    octx.strokeStyle = "rgba(126,207,179,0.38)"; octx.lineWidth = 1.8;
    octx.shadowColor = "rgba(126,207,179,0.25)"; octx.shadowBlur = 10;
    octx.stroke(); octx.shadowBlur = 0;

    this._torusCache      = oc;
    this._torusCacheValid = true;
    this._torusCacheSize  = diam;
  },
  ...
  drawRings(ctx, dt) {
    ...
    // Rebuild cache if radii changed significantly (pulse animation) or first run
    const diam = Math.ceil((Ro + 20) * 2);
    if (!this._torusCacheValid || Math.abs(diam - this._torusCacheSize) > 2) {
      this._buildTorusCache(Ro, Ri);
    }

    ctx.save();
    ctx.translate(cx, cy);

    // Draw cached base torus (static layers)
    const hw = this._torusCacheSize / 2;
    ctx.drawImage(this._torusCache, -hw, -hw);

    // Dynamic: shimmer band (rotates with torusAngle)
    ...
    // Dynamic: specular highlight dot
    ...
    // Dynamic: secondary specular (inner rim)
