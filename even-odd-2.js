const Game4 = {
  ...
  spawnBall() {
    ...
    const speed = 4 + (Math.random() * 2); // Slightly faster base speed

    let x, y, vx, vy;

    if (side === 0) { x = this.CENTER_X; y = -30; vx = (Math.random()-0.5)*3; vy = speed; } 
    else if (side === 1) { x = this.CENTER_X; y = canvasElement.height + 30; vx = (Math.random()-0.5)*3; vy = -speed; } 
    else if (side === 2) { x = -30; y = this.CENTER_Y; vx = speed; vy = (Math.random()-0.5)*3; } 
  ...
  drawBalls(ctx) {
    ...
    for (let b of this.balls) {
      ...
      if (b.trail.length > 1) {
          ...
          ctx.lineWidth = b.scale * this.BALL_RADIUS * 0.8;
          ctx.lineCap = "round";
          ctx.globalAlpha = 0.3;
